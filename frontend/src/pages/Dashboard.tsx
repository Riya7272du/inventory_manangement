    ...
    return (
        <div className="space-y-6" style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <section className="grid grid-cols-12 gap-4">
                <div className="col-span-3 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                ...
                <div className="col-span-3 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                    ...
                    <div className="text-3xl font-bold text-slate-100">${stats.stockValue.toLocaleString()}</div>
                ...
                <div className="col-span-3 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                    ...
                    <div className="text-3xl font-bold text-slate-100">{stats.lowStockCount}</div>
                ...
                <div className="col-span-3 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                    ...
                    <div className="text-3xl font-bold text-slate-100">{stats.supplierCount}</div>
                    <div className="text-xs text-slate-400 mt-1">Active suppliers</div>
                </div>
            </section>

            <section className="grid grid-cols-12 gap-4">
                <div className="col-span-8 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-slate-400 text-sm font-medium">Monthly Transactions</h3>
                        <button
                            onClick={() => window.location.href = '/reports'}
                            className="px-3 py-2 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-700 text-sm"
                        >
                            View report
                        </button>
                    </div>
                    <div className="flex items-end justify-center gap-3 h-44 p-3">
                        {stats.monthlyData.map((height, index) => (
                            <div
                                key={index}
                                className="w-10 bg-gradient-to-t from-blue-600 to-blue-500 rounded-t-lg shadow-lg"
                                style={{ height: `${height}%` }}
                            />
                        ))}
                    </div>
                ...
                <div className="col-span-4 p-4 border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
            ...
            <section className="border border-slate-600/50 rounded-xl" style={{ backgroundColor: '#161a2f' }}>
                <div className="flex justify-between items-center p-4 border-b border-slate-600/50">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse">
                        <thead>
                            <tr>
                                <th className="px-4 py-3 border-b border-slate-600/50 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Time</th>
                                <th className="px-4 py-3 border-b border-slate-600/50 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">User</th>
                                <th className="px-4 py-3 border-b border-slate-600/50 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Type</th>
                                <th className="px-4 py-3 border-b border-slate-600/50 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Item</th>
                                <th className="px-4 py-3 border-b border-slate-600/50 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Details</th>
                            </tr>
                        </thead>
                        <tbody>
                            {stats.recentTransactions.length > 0 ? (
                                stats.recentTransactions.map((transaction) => (
                                    <tr key={transaction.id} className="hover:bg-slate-700/30 transition-colors">
                                        <td className="px-4 py-3 border-b border-slate-600/50 text-slate-100 font-mono text-sm">
                                            {formatTime(transaction.formatted_date)}
                                        </td>
                                        <td className="px-4 py-3 border-b border-slate-600/50 text-slate-100">
                                            {transaction.user_name}
                                        </td>
                                        <td className="px-4 py-3 border-b border-slate-600/50">
